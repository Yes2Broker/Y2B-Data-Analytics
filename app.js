...
// Y2B: all rows. Custom: only rows whose Owner belongs to this company.
// Applies owner/source filters + date filter on Converted Date.
function getCompanyConverted(){
  let rows = (activeCompany===Y2B_ID)
    ? convertedCache
    : convertedCache.filter(r=>getCompanyOwners().has(r['Owner Name']));
  return rows;
}
...
function getCompanyVisits(){
  if(activeCompany===Y2B_ID){
    return visitsCache.map(r=>({
      mobile:r['Mobile No.']||'', by:r['Visit Done By']||'Unknown',
      date:r['Date']||'', location:r['Location']||'', project:r['Visited Project']||'',
      name:r['CustomerName']||'', _raw:r
    }));
  }
  // custom company: each masterdata lead with IsSiteVisitDone='Yes' = 1 visit
  return getCompanyLeads().filter(r=>r['IsSiteVisitDone']==='Yes').map(r=>({
    mobile:r['Mobile No.']||'', by:r['Owner Name']||'Unknown',
...
// Apply owner/date filters to the normalised visit list
function getFilteredVisits(){
  return getCompanyVisits().filter(v=>{
    if(fOwners.size && !fOwners.has(v.by)) return false;
    if(fFrom||fTo){
      const d=parseDate(v.date);if(!d)return false;
      if(fFrom&&d<new Date(fFrom))return false;
      if(fTo  &&d>new Date(fTo+'T23:59:59'))return false;
    }
    return true;
...
// Visits done count = total visit rows
function getVisitsDoneCount(){ return getFilteredVisits().length; }
// Revisits = mobiles appearing 2+ times in visit list (Y2B), or SiteVisitDoneCount>1 (custom)
function getRevisitCount(){
  ...
  return getFiltered().filter(r=>(parseInt(r['Site Visit Done Count'])||0)>1).length;
}
...
function renderPlatform(){
  ...
  mkChart('sourceBarChart','bar',lbl,val,{});
...
function renderOwner(){
  ...
  visits.forEach(v=>{
    visitsByOwner[v.by]=(visitsByOwner[v.by]||0)+1;
    if(!visitMobByOwner[v.by])visitMobByOwner[v.by]={};
    if(v.mobile)visitMobByOwner[v.by][v.mobile]=(visitMobByOwner[v.by][v.mobile]||0)+1;
  });
  ...
  mkChart('ownerBarChart','bar',lbl,lbl.map(l=>om[l].leads),{});
  ...
  const totals=['Total',d.length,d.reduce((s,r)=>s+callCount(r),0),
    m.visitsDone,
    m.revisits,
    d.filter(r=>r['Status Name']==='Inactive').length,'',totalConverted,
    m.visitRate+'%',m.closureRate+'%','','100%'];
...
function renderVisits(){
  ...
  if(!visits.length){
    ...
    if(el)el.innerHTML='<p style="padding:20px;color:#94a3b8">No visit data for this selection.</p>';
    ['visitStatusChart','revisitChart'].forEach(id=>{if(chartInstances[id])chartInstances[id].destroy();});
    ['visitProjectWrap','visitLocationWrap'].forEach(id=>{const w=document.getElementById(id);if(w)w.innerHTML='';});
  ...
  // Revisits by staff (mobiles 2+ for that staff)
...
function renderConverted(){
  ...
  const rows2=rows.slice((pg.page-1)*pg.size,pg.page*pg.size).map(r=>[
    ...
    `<span class="mono">${fmtDate(isY2B?r['Converted Date']:r['Created Date'])}</span>`,
...
function drillConv(type){
  ...
  const isY2B=activeCompany===Y2B_ID;
  const getName=r=>isY2B?r['CustomerName']||'—':`${r['FirstName']||''} ${r['LastName']||''}`.trim()||'—';
  const fmt=r=>[escHtml(getName(r)),`<span class="mono">${escHtml(r['Mobile No.']||'—')}</span>`,escHtml(r['Owner Name']||'—'),escHtml(r['Source Name']||'—'),`<span class="mono">${fmtDate(isY2B?r['Converted Date']:r['Created Date'])}</span>`,r['Site Visit Done Count']||0,parseInt(r['Make Call Count'])||1];
...
function renderComparison(){
  ...
  visits.forEach(v=>{
    visByOwner[v.by]=(visByOwner[v.by]||0)+1;
    if(!mobByOwner[v.by])mobByOwner[v.by]={};
    if(v.mobile)mobByOwner[v.by][v.mobile]=(mobByOwner[v.by][v.mobile]||0)+1;
  });
  ...
  const ownerOf=s=>s.length?s[0]['Owner Name']:null;
  const fn={
    leads:  s=>s.length,
    calls:  s=>s.reduce((t,r)=>t+callCount(r),0),
    visits: s=>visByOwner[ownerOf(s)]||0,
    revisit:s=>revisitByOwner[ownerOf(s)]||0,
    converted:s=>convByOwner[ownerOf(s)]||0,
    inactive:s=>s.filter(r=>r['Status Name']==='Inactive').length,
    visitRate:s=>{const c=s.reduce((t,r)=>t+callCount(r),0);const v=visByOwner[ownerOf(s)]||0;return c?Math.round(v/c*100):0;},
    closureRate:s=>{const v=visByOwner[ownerOf(s)]||0;const cv=convByOwner[ownerOf(s)]||0;return v?Math.round(cv/v*100):0;},
  }[metric]||(s=>s.length);
  ...
  fillTable('compTable',['Employee','Leads','Calls','Visits','Revisits','Converted','Visit Rate %','Closure Rate %','vs Top'],
    owners.map((o,i)=>{
      ...
      return[escHtml(o),sub.length,calls,vis,rev,cv,vr+'%',cr+'%',
...
function drillLC(type){
  ...
  if(type==='closed'){
    ...
    const isY2B=activeCompany===Y2B_ID;
    const getName=r=>isY2B?(r['CustomerName']||'—'):`${r['FirstName']||''} ${r['LastName']||''}`.trim()||'—';
    const fmt=r=>[escHtml(getName(r)),`<span class="mono">${escHtml(r['Mobile No.']||'—')}</span>`,escHtml(r['Owner Name']||'—'),escHtml(r['Source Name']||'—'),`<span class="mono">${fmtDate(isY2B?r['Converted Date']:r['Created Date'])}</span>`];
...
// ── ADMIN PANEL ───────────────────────────────────────────────
const ADMIN_PASSWORD = 'y2b@admin2025';   // change this to whatever you like
let adminUnlocked = false;

const ADMIN_TABLES = [
  { key:'master_data', label:'Master Data (Leads)', icon:'users',         desc:'All lead records' },
  { key:'site_visit',  label:'Site Visits',         icon:'map-pin',       desc:'Visit records (Y2B)' },
  { key:'convertion',  label:'Conversions',         icon:'check-circle-2',desc:'Closed/converted deals' },
];
...
function checkAdminPass(){
  ...
  if(val===ADMIN_PASSWORD){
    ...
    const inp=document.getElementById('adminPass');if(inp){inp.value='';inp.focus();}
  }
}

function renderAdmin(){
  // Reset to gate each time unless already unlocked this session
  if(adminUnlocked){
    document.getElementById('adminGate').style.display='none';
    document.getElementById('adminPanel').style.display='block';
    renderAdminGrid();
    ...
    const err=document.getElementById('adminPassErr');if(err)err.textContent='';
  }
  lucide.createIcons();
}
...
function renderAdminGrid(){
  ...
  const counts={ master_data:masterCache.length, site_visit:visitsCache.length, convertion:convertedCache.length };
  grid.innerHTML=ADMIN_TABLES.map(t=>`
    <div class="admin-card">
      <div class="admin-card-head">
        <div class="admin-card-icon"><i data-lucide="${t.icon}"></i></div>
        <div>
          <div class="admin-card-title">${t.label}</div>
          <div class="admin-card-desc">${t.desc}</div>
        </div>
        <div class="admin-card-count"><span class="mono">${(counts[t.key]||0).toLocaleString()}</span><small>rows</small></div>
      </div>
      <div class="admin-card-actions">
        <label class="admin-import-btn">
          <i data-lucide="upload" style="width:14px;height:14px"></i> Import CSV
          <input type="file" accept=".csv" style="display:none" onchange="adminImportCSV('${t.key}',this)"/>
        </label>
        <button class="admin-delete-btn" onclick="adminDeleteAll('${t.key}','${t.label}')">
          <i data-lucide="trash-2" style="width:14px;height:14px"></i> Delete All
        </button>
      </div>
      <div class="admin-progress" id="adminProg_${t.key}"></div>
    </div>`).join('');
  lucide.createIcons();
}

// Delete all rows in a table (id >= 0 catches all bigserial ids)
async function adminDeleteAll(table,label){
  showConfirm(`Delete ALL data in ${label}?`,`This permanently removes every row from "${table}". This cannot be undone. Export a backup first if needed.`,async()=>{
    const prog=document.getElementById(`adminProg_${table}`);
    if(prog)prog.innerHTML='<span class="admin-prog-text">Deleting…</span>';
    try{
      // delete all rows: id is bigserial >=1, so id>=0 matches everything
      await sbDeleteAdmin(table,'id=gte.0');
      showToast(`${label}: all data deleted`,'success');
      await loadData();           // reload caches + dashboard
      switchSection('admin');     // stay on admin
      renderAdminGrid();
    }catch(err){
      if(prog)prog.innerHTML=`<span class="admin-prog-err">Error: ${escHtml(err.message)}</span>`;
      showToast('Delete failed: '+err.message,'error');
    }
  });
}

// Dedicated delete that surfaces RLS errors clearly
async function sbDeleteAdmin(table,filter){
  const res=await fetch(`${SUPA_URL}/rest/v1/${table}?${filter}`,{method:'DELETE',headers:{...HEADERS,'Prefer':'return=minimal'}});
  if(!res.ok){
    let b='';try{b=await res.text();}catch(_){}
    if(res.status===401||res.status===403) throw new Error('Permission denied — the table needs a DELETE policy in Supabase (see setup note).');
    throw new Error(`[${res.status}] ${b||res.statusText}`);
  }
  return true;
}

// Import a CSV into a table (parses with SheetJS, batches inserts)
async function adminImportCSV(table,input){
  const file=input.files&&input.files[0];
  if(!file)return;
  const prog=document.getElementById(`adminProg_${table}`);
  const setProg=(html)=>{if(prog)prog.innerHTML=html;};
  setProg('<span class="admin-prog-text">Reading file…</span>');
  try{
    const buf=await file.arrayBuffer();
    const wb=XLSX.read(buf,{type:'array'});
    const ws=wb.Sheets[wb.SheetNames[0]];
    // Parse as array-of-objects keyed by header row, everything as text
    const rows=XLSX.utils.sheet_to_json(ws,{defval:'',raw:false});
    if(!rows.length){ setProg('<span class="admin-prog-err">No rows found in CSV</span>'); input.value=''; return; }

    // Strip phantom empty-header columns (e.g. "", "_1") and trim keys
    const clean=rows.map(r=>{
      const o={};
      Object.keys(r).forEach(k=>{
        const kk=String(k).trim();
        if(!kk||/^_+\d*$/.test(kk))return;     // skip blank / "_1" phantom cols
        o[kk]=r[k]===null?'':String(r[k]);
      });
      return o;
    });

    showConfirm(`Import ${clean.length.toLocaleString()} rows into ${table}?`,
      `This ADDS to existing data. To replace, delete all first. Detected columns: ${Object.keys(clean[0]).join(', ').slice(0,160)}…`,
      async()=>{
        try{
          const BATCH=500;
          let done=0;
          for(let i=0;i<clean.length;i+=BATCH){
            ...
            await sbPostAdmin(table,chunk);
            done+=chunk.length;
            setProg(`<div class="admin-prog-bar-wrap"><div class="admin-prog-bar" style="width:${Math.round(done/clean.length*100)}%"></div></div><span class="admin-prog-text">${done.toLocaleString()} / ${clean.length.toLocaleString()} rows</span>`);
          }
          showToast(`Imported ${done.toLocaleString()} rows into ${table}`,'success');
          await loadData();
          switchSection('admin');
          renderAdminGrid();
        }catch(err){
          setProg(`<span class="admin-prog-err">Import error: ${escHtml(err.message)}</span>`);
          showToast('Import failed: '+err.message,'error');
        }
      });
  }catch(err){
    setProg(`<span class="admin-prog-err">Could not read file: ${escHtml(err.message)}</span>`);
  }
  input.value='';   // reset so same file can be re-picked
}

async function sbPostAdmin(table,rows){
  const res=await fetch(`${SUPA_URL}/rest/v1/${table}`,{method:'POST',headers:{...HEADERS,'Content-Type':'application/json','Prefer':'return=minimal'},body:JSON.stringify(rows)});
  if(!res.ok){
    let b='';try{b=await res.text();}catch(_){}
    if(res.status===401||res.status===403) throw new Error('Permission denied — the table needs an INSERT policy in Supabase (see setup note).');
    throw new Error(`[${res.status}] ${b||res.statusText}`);
  }
  return true;
}
