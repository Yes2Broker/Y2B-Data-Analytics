/* ================================================================
   ...
   Revisit metric | Closure/Visit rates | Totals
   ================================================================ */

// ── CONFIG ────────────────────────────────────────────────────
// All data flows through the authenticated server proxy (/api/db). No Supabase
// keys live in the browser — the dashboard returns nothing without a valid session.
const DB_PROXY = '/api/db';
...
const STATUS_COLORS = {
  ...
  'Cold':'#64748b','Warm':'#f97316','Hot':'#dc2626','Converted':'#10b981',
};
...
let convertedCache = [];
let companies      = [];
let visitSourceConfig = {};   // normName(user) -> 'site_visit' | 'master_data' (overrides)
...
// ── DATA ACCESS (via authenticated /api/db proxy) ─────────────
// Cookies are sent automatically on these same-origin requests. A 401 means
// the session expired -> bounce back to the login gate.
async function dbProxy(resource,method='GET',query='',body=null){
  const ctrl=new AbortController();
  const t=setTimeout(()=>ctrl.abort(),20000);
  try{
    const res=await fetch(DB_PROXY,{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({resource,method,query,body}),signal:ctrl.signal});
    clearTimeout(t);
    if(res.status===401){showLogin();throw new Error('Session expired — please log in again.');}
    return res;
  }catch(e){clearTimeout(t);if(e.name==='AbortError')throw new Error(`Timeout on ${resource}`);throw e;}
}
async function sbFetch(table,params=''){
  const res=await dbProxy(table,'GET',params,null);
  if(!res.ok){let b='';try{b=await res.text();}catch(_){}throw new Error(`HTTP ${res.status}: ${b||res.statusText}`);}
  return res.json();
}
async function sbPost(table,body){
  const res=await dbProxy(table,'POST','',body);
  if(!res.ok){let b='';try{b=await res.text();}catch(_){}throw new Error(`[${res.status}] ${b}`);}
  return res.json();
}
async function sbDelete(table,filter){
  const res=await dbProxy(table,'DELETE',filter,null);
  if(!res.ok){let b='';try{b=await res.text();}catch(_){}throw new Error(`[${res.status}] ${b}`);}
  return true;
}
...
function showLogin(){
  ...
  if(el){el.style.display='flex';if(window.lucide)lucide.createIcons();setTimeout(()=>document.getElementById('loginPw')?.focus(),60);}
}
function hideLogin(){
  const el=document.getElementById('loginOverlay');
...
async function doLogin(){
  ...
  try{
    ...
    const d=await r.json();
    if(d.ok){ inp.value=''; hideLogin(); loadAll(); }
    else if(r.status===500){ err.textContent='Server auth is not configured. Contact the admin.'; }
    else { err.textContent='Incorrect password.'; inp.value=''; inp.focus(); }
  }catch(_){ err.textContent='Could not reach the server. Check your connection.'; }
...
const TITLES={overview:'Overview',platform:'Platform & Source',owner:'Owner-wise Leads',
  ...
  'leads-table':'All Leads',admin:'Admin',configuration:'Configuration'};
...
// ── SITE VISITS ───────────────────────────────────────────────
// Each visit is sourced PER USER, controlled from the Configuration page:
//   'site_visit'  → read from the site_visit table (attributed by "Visit Done By")
//   'master_data' → derived from master_data leads (IsSiteVisitDone='Yes', by Owner)
// When a user has no override, the company DEFAULT applies, which reproduces the
// original behaviour exactly:
//   yes2broker (Y2B) → site_visit ;  custom company → master_data
// Returns a normalised list: {mobile, owner, by, date, location, project, name, _raw}
function visitSourceFor(user){
  return visitSourceConfig[normName(user)] || (activeCompany===Y2B_ID ? 'site_visit' : 'master_data');
}
function getCompanyVisits(){
  const isY2B=activeCompany===Y2B_ID;
  const out=[];

  // (a) master_data-derived visits — for users routed to 'master_data'
  getCompanyLeads().forEach(r=>{
    if(r['IsSiteVisitDone']!=='Yes')return;
    const owner=normName(r['Owner Name']);
    if(visitSourceFor(owner)!=='master_data')return;
    out.push({
      mobile:normMobile(r['Mobile No.']), owner, by:owner,
  ...
  // (b) site_visit-table visits — for users routed to 'site_visit'
  // For a custom company, scope these rows to that company's projects.
  let companyProjSet=null;
  if(!isY2B){
    companyProjSet=new Set((companies.find(c=>String(c.id)===String(activeCompany))?.projects)||[]);
  }
  visitsCache.forEach(r=>{
    const by=normName(r['Visit Done By']);
    if(visitSourceFor(by)!=='site_visit')return;
    if(companyProjSet){
      const projs=splitProjects(r['Visited Project']);
      if(!projs.some(p=>companyProjSet.has(p)))return;
    }
    out.push({
      mobile:normMobile(r['Mobile No.']), owner:by, by,
...
// Filter visits by date + owner. Visit "owner" = "Visit Done By" (staff name).
// The Owners filter list includes visit-staff names, so selecting owners narrows
// visits to those staff. All visit data comes from the site_visit table.
...
async function loadAll(){
  ...
  try{
    ...
    visitSourceConfig = await loadVisitSourceConfig();
    allProjects =[...new Set(masterCache.map(r=>r['Project Name']).filter(Boolean))].sort();
    // Owners list = lead owners + visit staff + conversion owners (so the filter
    // covers every name that appears anywhere, including Site Visits "Visit Done By")
    {
      const set=new Set(masterCache.map(r=>r['Owner Name']).filter(Boolean));
      visitsCache.forEach(r=>{const n=normName(r['Visit Done By']);if(n&&n!=='Unknown')set.add(n);});
      convertedCache.forEach(r=>{if(r['Owner Name'])set.add(r['Owner Name']);});
      allOwners=[...set].sort();
    }
...
// Load the per-user visit-source overrides from Supabase. Keyed by normName.
async function loadVisitSourceConfig(){
  try{
    const rows=await sbFetch('visit_source_config','select=*');
    const map={};
    (rows||[]).forEach(r=>{
      if(r.user_name && (r.source==='site_visit'||r.source==='master_data')){
        map[normName(r.user_name)]=r.source;
      }
    });
    return map;
  }catch(_){ return {}; }
...
async function saveCompany(){
  ...
  try{
    ...
    else await dbProxy('companies','PATCH',`id=eq.${editId}`,{name});
...
// ── CHART FACTORIES ───────────────────────────────────────────
// Make the x-axis label names ("Facebook/Digital Marketing", "Unknown", …)
// clickable so users can drill in even when the bar is too small to hit.
// Chart.js doesn't expose tick-label clicks reliably, so we bind native
// listeners straight to the canvas and snap the pointer to the nearest tick.
//   canvas    : the chart's <canvas>
//   getChart  : () => current Chart instance (re-read each event; survives redraws)
//   getLabels : () => current category labels
//   cb        : (label, index) => void  — same drill handler used by bars
function attachLabelInteract(canvas, getChart, getLabels, cb){
  if(!canvas || !cb) return;
  // Keep the latest refs on the element so a single listener set always uses
  // current data even after the chart is destroyed & rebuilt on the same canvas.
  canvas._lblChart  = getChart;
  canvas._lblLabels = getLabels;
  canvas._lblCb     = cb;
  if(canvas._lblBound) return;            // listeners already attached once
  canvas._lblBound = true;

  // Which category sits under (clientX,clientY)? -1 if not in the label strip.
  const hitIndex = (clientX, clientY) => {
    const chart  = canvas._lblChart && canvas._lblChart();
    const labels = canvas._lblLabels ? canvas._lblLabels() : [];
    if(!chart || !labels || !labels.length) return -1;
    const xScale = chart.scales && chart.scales.x;
    if(!xScale) return -1;
    const rect = canvas.getBoundingClientRect();
    const x = clientX - rect.left, y = clientY - rect.top;
    // Vertical band: from just below the plot down to the canvas bottom, but
    // stop above a bottom legend so legend toggles aren't hijacked.
    const top = xScale.top - 2;
    let bottom = rect.height;
    const lg = chart.legend;
    if(lg && lg.options && lg.options.display && typeof lg.top==='number' && lg.top>xScale.top){
      bottom = Math.min(bottom, lg.top);
    }
    if(y < top || y > bottom) return -1;
    // Snap to nearest tick centre (rotation-proof, forgiving).
    let best=-1, bestDist=Infinity;
    for(let i=0;i<labels.length;i++){
      const px = xScale.getPixelForTick ? xScale.getPixelForTick(i) : xScale.getPixelForValue(i);
  ...
  canvas.addEventListener('click', (e)=>{
    const chart = canvas._lblChart && canvas._lblChart();
    if(!chart) return;
    // If a bar/point was clicked, Chart.js's own onClick handles it — skip.
    const onEl = chart.getElementsAtEventForMode(e,'nearest',{intersect:true},false);
    if(onEl && onEl.length) return;
    const idx = hitIndex(e.clientX, e.clientY);
    if(idx >= 0){
      const labels = canvas._lblLabels(), fn = canvas._lblCb;
      if(fn) fn(labels[idx], idx);
    }
  });

  // Pointer cursor when hovering a label (runs after Chart.js's own onHover,
  // so it wins for the label strip without disturbing bar-hover behaviour).
  canvas.addEventListener('mousemove', (e)=>{
    if(hitIndex(e.clientX, e.clientY) >= 0) canvas.style.cursor = 'pointer';
  });
}
...
function mkChart(id,type,labels,data,opts={}){
  ...
  // Round charts must render in a fixed-height, centered box so they stay perfect
  // circles (otherwise the global canvas max-height clips them into ovals).
  if(isRound&&!ctx.parentElement.classList.contains('pie-box')){
    const box=document.createElement('div');
    box.className='pie-box';
    ctx.parentNode.insertBefore(box,ctx);
    box.appendChild(ctx);
  }
  ...
  chartInstances[id]=new Chart(ctx,{
    ...
    options:{responsive:true,maintainAspectRatio:!isRound,animation:{duration:400,easing:'easeOutQuart'},
      ...
      plugins:{
        legend:{
          // Pie/doughnut: show a clickable legend so users can toggle (show/hide)
          // each slice. Chart.js handles the toggle + recalculates the chart.
          display:isRound||(isMulti&&type!=='bar'),
          position:'bottom',
          labels:{font:{family:FONT,size:11},padding:10,usePointStyle:true},
          onHover:isRound?(e)=>{e.native.target.style.cursor='pointer';}:undefined,
          onLeave:isRound?(e)=>{e.native.target.style.cursor='default';}:undefined,
        },
  ...
  // Make the x-axis label names clickable too (drill in even when the bar is tiny)
  if(opts.onBarClick) attachLabelInteract(ctx, ()=>chartInstances[id], ()=>labels, opts.onBarClick);
}
...
function mkScrollChart(wrapperId,type,allLabels,datasets,opts={}){
  ...
  // Click handler — fires with the clicked label when a bar/segment OR its
  // x-axis label (the bar "name") is clicked
  const onClickHandler = opts.onBarClick
    ? (evt, elements) => {
        if(elements && elements.length){
          opts.onBarClick(allLabels[elements[0].index], elements[0].index);
        }
      }
    : undefined;
  ...
  // x-axis label names clickable too (each scroll chart builds a fresh canvas)
  if(opts.onBarClick) attachLabelInteract(canvas, ()=>chartInstances[id], ()=>allLabels, opts.onBarClick);
...
const RENDERERS={overview:renderOverview,platform:renderPlatform,owner:renderOwner,
  ...
  'leads-table':renderLeadsTable,admin:renderAdmin,configuration:renderConfiguration};
...
function drillByStatus(status){
  // These two are sourced authoritatively in the Status section, so their drills
  // come from the same places (configured visits / convertion table), not master_data.
  if(status==='Site Visit Done'){ drillVisits('all'); return; }
  if(status==='Converted'){
    ...
    const rows=conv.map(r=>[
      ...
      escHtml(r['Project Name']||'—'),
      escHtml(r['Owner Name']||'—'),
      escHtml(r['Source Name']||'—'),
      `<span class="mono">${fmtDate(r['Converted Date']||r['Created Date'])}</span>`
    ]);
    showDrill(`Converted (${conv.length})`,rows,['Customer','Mobile','Project','Owner','Source','Date']);
    return;
  }
...
// Trend-chart period click → show the FULL lead list for that period (the bar's
// total), not a category breakdown. Used by both the Source Trend and Status
// Trend charts, where each bar's total is the leads created in that period.
function drillPeriodLeads(period){
  const d=getFiltered().filter(r=>{
    const dt=parseDate(r['Created Date']);
    return dt && bucketKey(dt)===period;
  });
  showDrill(`${period} — Leads (${d.length.toLocaleString()})`,d.map(leadRow),LEAD_H);
}
...
function renderOverview(){
  ...
  document.getElementById('kpiGrid').innerHTML=[
    ...
    kpiCard('flame',m.hot.toLocaleString(),'Hot Clients','red',`drillByStatus('Hot')`),
    kpiCard('repeat',m.revisits.toLocaleString(),'Revisits','cyan',`drillVisits('revisit')`),
    kpiCard('percent',m.visitRate+'%','Visit Rate','cyan','drillOv("visited")'),
    kpiCard('check-circle',m.closureRate+'%','Closure Rate','green','drillOv("closure")'),
    kpiCard('refresh-cw',m.revisitRate+'%','Revisit Rate','blue','drillVisits("revisit")'),
  ].join('');
...
function drillOv(type){
  ...
  const d=getFiltered();
  // Active Owners card -> show the owners themselves (count + share), not raw leads
  if(type==='owners'){
    const om={};
    d.forEach(r=>{const o=r['Owner Name'];if(o)om[o]=(om[o]||0)+1;});
    const owners=Object.keys(om).sort((a,b)=>om[b]-om[a]);
    const total=d.length;
    const rows=owners.map(o=>[
      `<span style="cursor:pointer;color:var(--blue-600);font-weight:600" onclick="drillByOwner('${escHtml(o).replace(/'/g,"\\'")}')">${escHtml(o)}</span>`,
      `<span class="mono">${om[o].toLocaleString()}</span>`,
      `<span class="mono">${total?Math.round(om[o]/total*100):0}%</span>`
    ]);
    showDrill(`Active Owners (${owners.length})`,rows,['Owner','Leads','Share %']);
    return;
  }
  // Total Calls card -> called leads with their call counts, reconciled to the card's total
  if(type==='calls'){
    const called=d.filter(r=>callCount(r)>0).sort((a,b)=>callCount(b)-callCount(a));
    const totalCalls=d.reduce((s,r)=>s+callCount(r),0);
    showDrill(`Total Calls: ${totalCalls.toLocaleString()} across ${called.length.toLocaleString()} leads`,called.map(leadRow),LEAD_H);
    return;
  }
  // Closure Rate card -> the actual closures (converted leads), matching converted/visits
  if(type==='closure'){
    const conv=getFilteredConverted();
    const getName=r=>(`${r['FirstName']||''} ${r['LastName']||''}`.trim())||r['CustomerName']||'—';
    const rows=conv.map(r=>[
    ...
    showDrill(`Closures (${conv.length})`,rows,['Customer','Mobile','Project','Owner','Source','Date']);
    return;
  }
  let rows=[],title='';
  switch(type){
...
function renderPlatform(){
  ...
  mkScrollChart('sourceTrendWrap','bar',tKeys,tDS,{stacked:true,pct:true,dl:{display:false},onBarClick:(period)=>drillPeriodLeads(period)});
...
function renderOwner(){
  ...
  if(ownerKpiEl) ownerKpiEl.innerHTML=[
    ...
    kpiCard('percent',m.closureRate+'%','Closure Rate','green','drillOwner("closure")'),
    kpiCard('refresh-cw',m.revisitRate+'%','Revisit Ratio','blue','drillOwner("revisit")'),
  ].join('');
...
// ── STATUS ────────────────────────────────────────────────────
// 'Site Visit Done' is sourced from the configured site-visit data (Configuration
// page decides each user's table) and 'Converted' from the convertion table.
// All other statuses come from master_data's Status Name as usual.
function renderStatus(){
  const d=getFiltered();setBadge('statusCount',d.length);

  // Configured site visits — count every visit event (matches the Site Visits page)
  const visits=getFilteredVisits();
  // Conversions straight from the convertion table
  const convList=getFilteredConverted();
  const SRC=['Site Visit Done','Converted'];   // statuses we source authoritatively

  // Status counts
  const sm={};
  d.forEach(r=>{const s=r['Status Name']||'Unknown';if(SRC.includes(s))return;sm[s]=(sm[s]||0)+1;});
  if(visits.length)   sm['Site Visit Done']=visits.length;
  if(convList.length) sm['Converted']=convList.length;
  ...
  const total=val.reduce((a,b)=>a+b,0)||1;

  // KPI cards for each status — exclude the two site-visit statuses, and always
  // end with a dedicated Hot Leads card.
  const statusKpiEl=document.getElementById('statusKpiGrid');
  if(statusKpiEl){
    const HIDE=['Site Visit Schedule','Site Visit Done','Hot'];
    const dyn=lbl.filter(s=>!HIDE.includes(s)).slice(0,5);
    const cards=dyn.map(s=>{
      const cls=s==='Inactive'?'red':s==='Converted'?'green':s==='Warm'?'amber':s==='Cold'?'':'blue';
      return kpiCard('activity',sm[s].toLocaleString(),s,cls,`drillByStatus('${escHtml(s)}')`);
    });
    cards.push(kpiCard('flame',(sm['Hot']||0).toLocaleString(),'Hot Leads','red',`drillByStatus('Hot')`));
    statusKpiEl.innerHTML=cards.join('');
  }
  ...
  // Status by Owner — same per-user sourcing
  const byOS={};
  const addOS=(s,o)=>{o=o||'Unknown';if(!byOS[s])byOS[s]={};byOS[s][o]=(byOS[s][o]||0)+1;};
  d.forEach(r=>{const s=r['Status Name']||'Unknown';if(SRC.includes(s))return;addOS(s,r['Owner Name']);});
  visits.forEach(v=>addOS('Site Visit Done',v.owner));
  convList.forEach(r=>addOS('Converted',r['Owner Name']));
  const owners=[...new Set(Object.values(byOS).flatMap(o=>Object.keys(o)))].sort();
  const sOwnerDS=ordered.filter(s=>byOS[s]).map(s=>({label:s,data:owners.map(o=>(byOS[s]&&byOS[s][o])||0),backgroundColor:STATUS_COLORS[s]||'#94a3b8',borderWidth:0}));
  mkScrollChart('statusOwnerWrap','bar',owners,sOwnerDS,{stacked:true,onBarClick:(owner)=>drillByOwner(owner)});

  // Status Trend — leads by Created Date, visits by visit date, conversions by Converted Date
  const tMap={};
  const bump=(key,s)=>{if(!key)return;if(!tMap[key])tMap[key]={};tMap[key][s]=(tMap[key][s]||0)+1;};
  d.forEach(r=>{const s=r['Status Name']||'Unknown';if(SRC.includes(s))return;const dt=parseDate(r['Created Date']);if(dt)bump(bucketKey(dt),s);});
  visits.forEach(v=>{const dt=parseDate(v.date);if(dt)bump(bucketKey(dt),'Site Visit Done');});
  convList.forEach(r=>{const dt=parseDate(r['Converted Date']||r['Created Date']);if(dt)bump(bucketKey(dt),'Converted');});
  const tKeys=sortBuckets(Object.keys(tMap));
  const stDS=ordered.filter(s=>Object.values(tMap).some(t=>t[s])).map(s=>({label:s,data:tKeys.map(k=>(tMap[k]&&tMap[k][s])||0),backgroundColor:STATUS_COLORS[s]||'#94a3b8',borderWidth:0}));
  mkScrollChart('statusTrendWrap','bar',tKeys,stDS,{stacked:true,onBarClick:(period)=>drillPeriodLeads(period)});

  const pg=getPg('statusTable');
  const rows=lbl.slice((pg.page-1)*pg.size,pg.page*pg.size).map(l=>[badge(l),`<span class="mono">${sm[l].toLocaleString()}</span>`,Math.round(sm[l]/total*100)+'%']);
  fillTable('statusTable',['Status','Count','Share %'],rows,['Total',total.toLocaleString(),'100%']);
...
function renderVisits(){
  ...
  const projectCount=new Set(visits.map(v=>v.project&&v.project.trim()).filter(Boolean)).size;
  const uniqueLocs=new Set(visits.map(v=>v.location&&v.location.trim()).filter(Boolean)).size;
  ...
  document.getElementById('visitKpiGrid').innerHTML=[
    ...
    kpiCard('check-circle',projectCount,'Project Count','','drillVisitProjects()'),
    kpiCard('compass',uniqueLocs,'Location Count','cyan','drillVisitLocations()'),
  ].join('');
...
// Distinct projects we've visited, with visit count each
function drillVisitProjects(){
  const m={};
  getFilteredVisits().forEach(v=>{ const p=(v.project||'').trim(); if(p) m[p]=(m[p]||0)+1; });
  const rows=Object.keys(m).sort((a,b)=>m[b]-m[a]).map(p=>[escHtml(p),`<span class="mono">${m[p].toLocaleString()}</span>`]);
  showDrill(`Projects Visited (${rows.length})`,rows,['Project','Visits']);
}
// Distinct locations we've visited, with visit count each
function drillVisitLocations(){
  const m={};
  getFilteredVisits().forEach(v=>{ const l=(v.location||'').trim(); if(l) m[l]=(m[l]||0)+1; });
  const rows=Object.keys(m).sort((a,b)=>m[b]-m[a]).map(l=>[escHtml(l),`<span class="mono">${m[l].toLocaleString()}</span>`]);
  showDrill(`Locations Visited (${rows.length})`,rows,['Location','Visits']);
}
...
function renderLifecycle(){
  ...
  // Hero funnel band — headline of the whole journey: Leads In → Closures
  const overallConv=total?Math.round(converted/total*100):0;
  const hero=`
    <div class="lc-hero">
      <div class="lc-hero-end">
        <div class="lc-hero-num">${total.toLocaleString()}</div>
        <div class="lc-hero-lbl"><i data-lucide="inbox" style="width:12px;height:12px"></i> Leads In</div>
      </div>
      <div class="lc-hero-flow">
        <div class="lc-hero-track"><div class="lc-hero-fill" style="width:${Math.max(overallConv,3)}%"></div></div>
        <div class="lc-hero-conv">${overallConv}% overall conversion</div>
      </div>
      <div class="lc-hero-end">
        <div class="lc-hero-num accent">${converted.toLocaleString()}</div>
        <div class="lc-hero-lbl">Closures <i data-lucide="check-circle" style="width:12px;height:12px"></i></div>
      </div>
    </div>`;

  // Numbered spine of stage cards, each with a conversion ring (or cumulative tally)
  const spine=stages.map((s,i)=>{
    const num=String(i+1).padStart(2,'0');
    const meter=s.pct!==null
      ? `<div class="lc-ring" style="--p:${Math.min(s.pct,100)}"><span>${s.pct}%</span></div>`
      : `<div class="lc-tally" title="Cumulative count">&Sigma;</div>`;
    return `
    <div class="lc-stage" onclick="${s.drillFn.replace(/"/g,"'")}" style="--c:${s.color};--tint:${s.color}14;--ring-track:${s.color}26;animation-delay:${i*0.07}s">
      <div class="lc-rail"><span class="lc-rail-num">${num}</span></div>
      <div class="lc-node">
        <div class="lc-orb"><i data-lucide="${s.icon}"></i></div>
        <div class="lc-body">
          <div class="lc-label">${s.label}</div>
          <div class="lc-val">${s.val.toLocaleString()}</div>
          <div class="lc-desc">${s.desc}</div>
        </div>
        ${meter}
        <i data-lucide="arrow-up-right" class="lc-go"></i>
      </div>
    </div>`;
  }).join('');

  lcWrap.innerHTML=hero+`<div class="lc-spine">${spine}</div>`;
...
function buildMascot(closureRate){
  ...
  <div class="lc-mascot-wrap">
    <div class="lc-coach-head"><i data-lucide="sparkles" style="width:13px;height:13px"></i> Deal Coach</div>
...
// ── CONFIGURATION ─────────────────────────────────────────────
// First feature: per-user visit-data source (site_visit table vs master_data).
let _cfgSel={};   // working selections while the page is open: user -> 'default'|'site_visit'|'master_data'

// Every person who could "do" a visit: lead owners + site_visit "Visit Done By"
function getAllVisitUsers(){
  const set=new Set();
  masterCache.forEach(r=>{ if(r['Owner Name']) set.add(normName(r['Owner Name'])); });
  visitsCache.forEach(r=>{ const n=normName(r['Visit Done By']); if(n&&n!=='Unknown') set.add(n); });
  set.delete('Unknown');
...
function renderConfiguration(){
  const sec=document.getElementById('section-configuration');
  if(!sec) return;
  // seed working selections from the saved config each time the page opens
  _cfgSel={};
  getAllVisitUsers().forEach(u=>{ _cfgSel[u]=visitSourceConfig[u]||'default'; });

  sec.innerHTML=`
    <div class="sec-head"><h2 class="sec-title">Configuration</h2></div>
    <div class="chart-card cfg-card">
      <div class="cl"><i data-lucide="database" class="cl-i"></i>Visit Data Source — per User</div>
      <p class="cfg-help">
        Choose which table each user's <strong>site-visit</strong> data is read from.
        <em>Default</em> follows the company rule — yes2broker reads the <strong>site_visit</strong> table,
        custom companies derive visits from <strong>master_data</strong>. Set an explicit source for any
        user whose visits live in the other table.
      </p>
      <div class="cfg-toolbar">
        <div class="cfg-search">
          <i data-lucide="search" style="width:13px;height:13px;color:#94a3b8"></i>
          <input id="cfgSearch" type="text" placeholder="Search user…" oninput="renderCfgList(this.value)"/>
        </div>
        <div class="cfg-bulk">
          <span>Set shown to:</span>
          <button class="cfg-bulk-btn" onclick="cfgBulk('default')">Default</button>
          <button class="cfg-bulk-btn" onclick="cfgBulk('site_visit')">Site Visit</button>
          <button class="cfg-bulk-btn" onclick="cfgBulk('master_data')">Master Data</button>
        </div>
      </div>
      <div id="cfgList" class="cfg-list"></div>
      <div class="cfg-footer">
        <span class="cfg-dirty" id="cfgDirty"></span>
        <button class="btn-primary" id="cfgSaveBtn" onclick="saveVisitSourceConfig()">Save Configuration</button>
      </div>
    </div>`;
  renderCfgList('');
  lucide.createIcons();
...
function renderCfgList(search){
  const wrap=document.getElementById('cfgList'); if(!wrap) return;
  const q=(search||'').toLowerCase().trim();
  const users=Object.keys(_cfgSel).sort((a,b)=>a.localeCompare(b)).filter(u=>!q||u.toLowerCase().includes(q));
  if(!users.length){ wrap.innerHTML='<p class="cfg-empty">No users match.</p>'; updateCfgDirty(); return; }
  const opt=(cur,val,label)=>`<option value="${val}"${cur===val?' selected':''}>${label}</option>`;
  wrap.innerHTML=users.map(u=>{
    const v=_cfgSel[u]||'default';
    const tag=v==='site_visit'?'<span class="cfg-tag sv">site_visit</span>'
            :v==='master_data'?'<span class="cfg-tag md">master_data</span>'
            :'<span class="cfg-tag df">default</span>';
    const safe=escHtml(u).replace(/'/g,"\\'");
    return `<div class="cfg-row">
        <span class="cfg-user"><i data-lucide="user" style="width:13px;height:13px;color:#94a3b8"></i>${escHtml(u)}</span>
        ${tag}
        <select class="cfg-select" onchange="cfgSet('${safe}',this.value)">
          ${opt(v,'default','Default')}
          ${opt(v,'site_visit','Site Visit table')}
          ${opt(v,'master_data','Master Data table')}
        </select>
      </div>`;
  }).join('');
  lucide.createIcons();
...
function cfgSet(user,val){ _cfgSel[user]=val; renderCfgList(document.getElementById('cfgSearch')?.value||''); }
function cfgBulk(val){
  const q=(document.getElementById('cfgSearch')?.value||'').toLowerCase().trim();
  Object.keys(_cfgSel).forEach(u=>{ if(!q||u.toLowerCase().includes(q)) _cfgSel[u]=val; });
  renderCfgList(document.getElementById('cfgSearch')?.value||'');
}
function updateCfgDirty(){
  let changed=0;
  Object.keys(_cfgSel).forEach(u=>{ if(_cfgSel[u]!==(visitSourceConfig[u]||'default')) changed++; });
  const el=document.getElementById('cfgDirty');
  if(el) el.textContent=changed?`${changed} unsaved change${changed>1?'s':''}`:'All changes saved';
  const btn=document.getElementById('cfgSaveBtn');
  if(btn) btn.classList.toggle('cfg-has-changes',changed>0);
}
async function saveVisitSourceConfig(){
  const btn=document.getElementById('cfgSaveBtn');
  if(btn){ btn.disabled=true; btn.textContent='Saving…'; }
  try{
    // Store only explicit overrides; rebuild the whole table each save.
    const rows=Object.keys(_cfgSel)
      .filter(u=>_cfgSel[u]==='site_visit'||_cfgSel[u]==='master_data')
      .map(u=>({user_name:u,source:_cfgSel[u]}));
    await sbDelete('visit_source_config','id=gte.0');
    if(rows.length) await sbPost('visit_source_config',rows);
    // refresh in-memory config + recompute everything that depends on visits
    visitSourceConfig={};
    rows.forEach(r=>{ visitSourceConfig[normName(r.user_name)]=r.source; });
    invalidate(); invalidateMobileIndex();
    updateCfgDirty();
    showToast('Configuration saved — visit numbers updated','success');
  }catch(err){ showToast('Save failed: '+err.message,'error'); }
  finally{ if(btn){ btn.disabled=false; btn.textContent='Save Configuration'; } }
}
