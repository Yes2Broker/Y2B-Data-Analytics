...
// ── CHART FACTORIES ───────────────────────────────────────────
// Detect a click/hover landing on the x-axis tick-label strip (the bar "names"
// drawn below the plot). Returns the category index, or -1 when the pointer is
// over the plot area / legend / outside. Lets users click a label to drill in.
function axisLabelIndex(chart,evt){
  const xScale=chart&&chart.scales&&chart.scales.x;
  if(!xScale)return -1;
  const x=evt.x,y=evt.y;
  if(x==null||y==null)return -1;
  // Vertical band = the x-axis itself (excludes plot area above & bottom legend below)
  if(y<xScale.top||y>xScale.bottom)return -1;
  // Horizontal band = the plotting width (so category mapping lines up with bars)
  if(x<chart.chartArea.left||x>chart.chartArea.right)return -1;
