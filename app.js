...
// State model: empty Set = nothing selected (NOT "all selected")
// We track explicitly. "Select All" = add all to set.
...
function buildMultiSelect(wrapperId, options, selectedSet, label) {
  const wrap = document.getElementById(wrapperId);
  if (!wrap) return;
  const count = selectedSet.size;
  const btnTxt = count > 0 ? `${label} <span class="ms-count">${count}</span>` : label;
  wrap.innerHTML = `
    <div class="ms-wrap" onclick="event.stopPropagation()">
      <button class="ms-btn${count > 0 ? ' ms-active' : ''}"
              onclick="toggleMsDropdown('${wrapperId}', event)">
        ${btnTxt}
      ...
      <div class="ms-dropdown" id="ms_${wrapperId}" onclick="event.stopPropagation()">
        ...
        <div class="ms-options" id="mso_${wrapperId}">
          ${renderMsOptions(options, selectedSet, wrapperId)}
        </div>
        <div class="ms-footer">
          <span class="ms-sel-count" id="msc_${wrapperId}">
            ${count > 0 ? count + ' selected' : 'None selected'}
          </span>
          <button class="ms-clear" onclick="clearMsFilter('${wrapperId}');event.stopPropagation()">
            Clear
          </button>
          <button class="ms-apply"
                  onclick="closeMsDropdown('${wrapperId}');applyFilters();event.stopPropagation()">
            Apply
          </button>
        </div>
      </div>
...
function renderMsOptions(options, selectedSet, wrapperId, filter = '') {
  const list = filter
    ? options.filter(o => o.toLowerCase().includes(filter.toLowerCase()))
    : options;

  // "Select All" is checked only if ALL options are explicitly in the set
  const allChecked = selectedSet.size > 0 && options.every(o => selectedSet.has(o));
  const noneChecked = selectedSet.size === 0;

  let html = `
    <div class="ms-opt ms-opt-all${allChecked ? ' ms-checked' : ''}"
         onclick="toggleMsAll('${wrapperId}', ${JSON.stringify(options)});event.stopPropagation()">
      <span class="ms-chk-box">${allChecked ? '<svg viewBox="0 0 12 12"><polyline points="2,6 5,9 10,3" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>' : ''}</span>
      <span class="ms-opt-label">Select All</span>
      <span class="ms-opt-badge" style="margin-left:auto;font-size:10px;color:var(--s400)">${options.length}</span>
    </div>
  ...
  list.forEach(o => {
    ...
    html += `
      <div class="ms-opt${checked ? ' ms-checked' : ''}"
        ...
        <span class="ms-chk-box">${checked ? '<svg viewBox="0 0 12 12"><polyline points="2,6 5,9 10,3" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>' : ''}</span>
...
function toggleMsDropdown(wrapperId, event) {
  ...
  if (!isOpen) {
    ...
    // Focus search
    setTimeout(() => d.querySelector('.ms-search')?.focus(), 50);
  }
}

function closeMsDropdown(wrapperId) {
...
function getLabelForWrapper(wrapperId) {
  return wrapperId.includes('Owner') ? 'Owners'
       : wrapperId.includes('Source') ? 'Sources' : 'Statuses';
}

function toggleMsAll(wrapperId, options) {
  const { set } = getMsState(wrapperId);
  const allOpts = options || getMsState(wrapperId).all;
  const allChecked = allOpts.every(o => set.has(o));
  if (allChecked) {
    // All selected → unselect all
    set.clear();
  } else {
    // Not all selected → select all
    allOpts.forEach(o => set.add(o));
  }
...
function refreshMsDropdown(wrapperId) {
  ...
  // Update counter
  const counter = document.getElementById(`msc_${wrapperId}`);
  if (counter) counter.textContent = set.size > 0 ? set.size + ' selected' : 'None selected';

  // Update button
  const lbl = getLabelForWrapper(wrapperId);
  const btn = document.querySelector(`#${wrapperId} .ms-btn`);
  if (btn) {
    btn.innerHTML = set.size > 0
      ? `${lbl} <span class="ms-count">${set.size}</span><i data-lucide="chevron-down" class="ms-chevron" id="msChev_${wrapperId}" style="width:11px;height:11px;flex-shrink:0;pointer-events:none;transform:rotate(180deg)"></i>`
      : `${lbl}<i data-lucide="chevron-down" class="ms-chevron" id="msChev_${wrapperId}" style="width:11px;height:11px;flex-shrink:0;pointer-events:none"></i>`;
    btn.classList.toggle('ms-active', set.size > 0);
    lucide.createIcons();
  }
}
...
function mkScrollChart(wrapperId,type,allLabels,datasets,opts={}){
  ...
  // Default datalabels for bar scroll charts
  const defaultDl = type==='bar'
...
function renderPlatform(){
  ...
  mkScrollChart('sourceTrendWrap','bar',tKeys,tDS,{stacked:true,pct:true,dl:{display:false}});
...
function renderOwner(){
  ...
  if(ownerKpiEl) ownerKpiEl.innerHTML=[
    ...
    kpiCard('percent',m.visitRate+'%','Visit Rate','green'),
    kpiCard('percent',m.closureRate+'%','Closure Rate','green'),
    paidDataCard(d),
  ].join('');
...
function drillOwner(type){
  ...
  switch(type){
    case 'all':     rows=d;title='All Leads';break;
    case 'owners':  rows=d;title='Leads by Owner';break;
    case 'visited': rows=d.filter(r=>r['IsSiteVisitDone']==='Yes');title='Site Visits Done';break;
