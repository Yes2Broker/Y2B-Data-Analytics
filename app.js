...
// Filter visits by date + owner. The owner multi-select lists LEAD owners,
// but visit "owner" = Visit Done By (staff). These are different name sets,
// so the owner filter only narrows visits when at least one selected name
// is actually a visit-staff name — otherwise it would wipe all visits.
function getFilteredVisits(){
  const all=getCompanyVisits();
  let applyOwner=false;
  if(fOwners.size){
    const staffSet=new Set(all.map(v=>v.owner));
    applyOwner=[...fOwners].some(o=>staffSet.has(o));
  }
  return all.filter(v=>{
    if(applyOwner && !fOwners.has(v.owner)) return false;
    if(fFrom||fTo){
