...
// Apply filters to the normalised visit list.
// NOTE: "Visit Done By" (staff) is a different dimension than lead "Owner Name".
// The owner multi-select only narrows visits when the selected owner names
// actually appear as visit staff — otherwise it would wipe all visits.
function getFilteredVisits(){
  const ownerFilterActive = fOwners.size>0;
  // Which selected owners actually appear as visit staff?
  const staffNames = new Set(getCompanyVisits().map(v=>(v.by||'').trim()));
  const ownerMatchesAnyStaff = ownerFilterActive && [...fOwners].some(o=>staffNames.has((o||'').trim()));

  return getCompanyVisits().filter(v=>{
    // Only filter by staff if the selected owners are actually visit-staff names
    if(ownerFilterActive && ownerMatchesAnyStaff && !fOwners.has((v.by||'').trim())) return false;
    if(fFrom||fTo){
