...
// ── CHART FACTORIES ───────────────────────────────────────────
// Detect a click/hover landing on the x-axis label strip (the bar "names" drawn
// below the plot) and return the matching category index, or -1. Uses
// nearest-tick-by-x across the whole label area, so it stays accurate even with
// long, 45°-rotated labels whose text sits offset from its tick. This lets users
// drill in by clicking a label name even when the bar itself is tiny to hit.
function axisLabelIndex(chart,evt){
  const xScale=chart&&chart.scales&&chart.scales.x;
  if(!xScale)return -1;
  const x=evt.x,y=evt.y;
  if(x==null||y==null)return -1;
  // Clickable band: from just below the plot area down to the canvas bottom —
  // but stop above a bottom legend so legend toggles aren't hijacked.
  const top=xScale.top-2;
  let bottom=chart.height;
  ...
  // Snap to the nearest tick centre (forgiving + rotation-proof).
...
function mkChart(id,type,labels,data,opts={}){
  ...
  chartInstances[id]=new Chart(ctx,{
    ...
    options:{responsive:true,maintainAspectRatio:!isRound,animation:{duration:400,easing:'easeOutQuart'},
      ...
      onClick:(opts.onBarClick||opts.onSliceClick)?(evt,els,chart)=>{
        const cb=opts.onBarClick||opts.onSliceClick;
        if(els&&els.length){cb(labels[els[0].index],els[0].index);return;}
        if(opts.onBarClick){const li=axisLabelIndex(chart,evt);if(li>=0)opts.onBarClick(labels[li],li);}
      }:undefined,
      onHover:(opts.onBarClick||opts.onSliceClick)?(e,el,chart)=>{
        let pointer=el.length>0;
        if(!pointer&&opts.onBarClick)pointer=axisLabelIndex(chart,e)>=0;
        e.native.target.style.cursor=pointer?'pointer':'default';
      }:undefined,
      plugins:{
        legend:{
...
function mkScrollChart(wrapperId,type,allLabels,datasets,opts={}){
  ...
  const onClickHandler = opts.onBarClick
    ? (evt, elements, chart) => {
        ...
        const li = axisLabelIndex(chart, evt);
        if(li >= 0) opts.onBarClick(allLabels[li], li);
      }
    : undefined;
  ...
  chartInstances[id]=new Chart(canvas,{
    ...
    options:{
      ...
      onHover: opts.onBarClick ? (e,el,chart)=>{ let p=el.length>0; if(!p)p=axisLabelIndex(chart,e)>=0; e.native.target.style.cursor = p ? 'pointer' : 'default'; } : undefined,
      plugins:{
