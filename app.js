...
function getCompanyVisits(){
  if(activeCompany===Y2B_ID){
    const idx=getMobileOwnerIndex();
    return visitsCache.map(r=>{
      const m=normMobile(r['Mobile No.']);
      return {
        mobile:m,
        owner: idx[m] || 'Unassigned',
        by: normName(r['Visit Done By']),
...
function renderOwner(){
  ...
  // Note any visits not attributable to a listed owner (mobile matched no lead)
  const unmatched=m.visitsDone-sumVisits;
  const note=document.getElementById('ownerVisitNote');
  if(note){
    note.innerHTML = unmatched>0
      ? `<i data-lucide="info" style="width:12px;height:12px"></i> ${m.visitsDone.toLocaleString()} total visits in site_visit; ${sumVisits.toLocaleString()} matched to a lead owner by mobile, ${unmatched.toLocaleString()} unmatched (no lead with that mobile).`
      : '';
    lucide.createIcons();
  }
