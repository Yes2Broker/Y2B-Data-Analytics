...
function renderStatus(){
  ...
  // Configured site visits, de-duplicated to one per lead (mobile)
  const visitSeen=new Set(),distinctVisits=[];
  getFilteredVisits().forEach(v=>{ if(v.mobile){ if(visitSeen.has(v.mobile))return; visitSeen.add(v.mobile);} distinctVisits.push(v); });
  ...
  if(distinctVisits.length) sm['Site Visit Done']=distinctVisits.length;
  ...
  distinctVisits.forEach(v=>addOS('Site Visit Done',v.owner));
  ...
  distinctVisits.forEach(v=>{const dt=parseDate(v.date);if(dt)bump(bucketKey(dt),'Site Visit Done');});
