...
function buildMultiSelect(wrapperId, options, selectedSet, label){
  ...
  wrap.innerHTML=`
    <div class="ms-wrap" onclick="event.stopPropagation()">
      <button class="ms-btn${activeCount>0?' ms-active':''}" onclick="toggleMsDropdown('${wrapperId}',event)">
        ${escHtml(btnLabel)}<i data-lucide="chevron-down" style="width:11px;height:11px;margin-left:4px;pointer-events:none"></i>
      </button>
      <div class="ms-dropdown" id="ms_${wrapperId}" onclick="event.stopPropagation()">
        <div class="ms-search-wrap">
          <input type="text" placeholder="Search…" oninput="filterMsOptions('${wrapperId}',this.value)" class="ms-search" onclick="event.stopPropagation()"/>
        </div>
        <div class="ms-options" id="mso_${wrapperId}">
          ${renderMsOptions(options, selectedSet, wrapperId)}
        </div>
        <div class="ms-footer">
          <button class="ms-clear" onclick="clearMsFilter('${wrapperId}');event.stopPropagation()">Clear</button>
          <button class="ms-apply" onclick="closeMsDropdown('${wrapperId}');applyFilters();event.stopPropagation()">Apply</button>
        </div>
      </div>
    </div>`;
  lucide.createIcons();
...
function renderMsOptions(options, selectedSet, wrapperId, filter=''){
  ...
  let html=`<div class="ms-opt${allSelected?' ms-checked':''}" onclick="toggleMsAll('${wrapperId}');event.stopPropagation()">
    <span class="ms-chk">${allSelected?'✓':''}</span><span>Select All</span>
  </div>`;
  filtered.forEach(o=>{
    const checked=selectedSet.size===0||selectedSet.has(o);
    html+=`<div class="ms-opt${checked?' ms-checked':''}" onclick="toggleMsOption('${wrapperId}','${escHtml(o).replace(/'/g,"\\'")}');event.stopPropagation()">
...
// Only close when clicking completely outside the filter bar
...
function refreshMsDropdown(wrapperId){
  ...
  if(btn){
    // Rebuild inner content to keep icon
    btn.innerHTML=`${set.size>0?`${lbl} (${set.size})`:lbl}<i data-lucide="chevron-down" style="width:11px;height:11px;margin-left:4px;pointer-events:none"></i>`;
