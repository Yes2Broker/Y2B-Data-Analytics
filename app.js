...
function renderVisits(){
  ...
  if(isY2B){
    ...
    document.getElementById('visitKpiGrid').innerHTML=[
      kpiCard('map-pin',vr.length.toLocaleString(),'Site Visits Done','green','drillVisits("revisit")'),
      kpiCard('repeat',revisitLeads.length.toLocaleString(),'Revisits','amber','drillVisits("revisit")'),
    ].join('');
    const pg=getPg('visitTable');
...
function drillVisits(type){
  const rl=getFiltered().filter(r=>(parseInt(r['Site Visit Done Count'])||0)>1);
  // Custom companies only have masterdata-based revisit data
  if(activeCompany!==Y2B_ID){
    showDrill(`Revisit Leads (${rl.length})`,rl.map(leadRow),LEAD_H);
    return;
  }
  const vr=visitsCache;
