/* ================================================================
   YES2BROKER — db-manager.js  v2
   Fixes: XLSX support, in-app confirm modal, preview table layout
   ================================================================ */
        ...
        <div class="db-steps">
          ...
          <div class="db-action-card danger">
            ...
            <button class="db-btn-danger" id="dbTruncateBtn" onclick="dbTruncate()">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg>
              Delete All Data
            </button>
          </div>
          ...
          <div class="db-action-card">
            <div class="db-ac-head">
              ...
              <div>
                ...
                <div class="db-ac-sub">Insert new data from a CSV or Excel file</div>
              </div>
            </div>
            ...
            <div class="db-drop-zone" id="dbDropZone"
              ...
              <span id="dbDropLabel">Click to browse or drag &amp; drop</span>
...
async function renderDbTableCards() {
  ...
  wrap.innerHTML = Object.entries(DB_TABLES).map(([key, t]) => `
    <div class="db-table-card db-color-${t.color}" onclick="dbOpenWorkspace('${key}')">
      ...
      <div class="db-tc-count" id="dbCount_${key}"><span class="db-count-spinner"></span></div>
...
async function dbVerifyPassword() {
  ...
  } catch { err.textContent = 'Could not reach server. Check your connection.'; }
  btn.disabled = false;
...
function dbCloseWorkspace() {
  ...
  dbCurrentTable = null; dbClearFile();
}
...
async function dbTruncate() {
  ...
  btn.disabled = true;
  btn.innerHTML = `<span class="db-count-spinner" style="border-color:rgba(255,255,255,.3);border-top-color:#fff"></span> Deleting…`;
  ...
  } catch (e) { dbShowInlineError(`Network error: ${e.message}`); }
...
function dbReadCSV(file) {
  ...
  reader.onload = e => {
    ...
    dbParsedRows = rows;
    dbShowPreview(headers.map(h => h.trim()), rows);
  };
  reader.readAsText(file);
}

// XLSX reader — uses SheetJS loaded from CDN
...
function dbParseXLSX(file) {
  ...
  reader.onload = e => {
    try {
      ...
      // Normalize: trim all keys and values
...
function dbShowPreview(headers, rows) {
    ...
    errWrap.innerHTML = `
      <div class="db-error-box">
        <div class="db-error-title">
          ...
          ${errors.length} validation issue${errors.length > 1 ? 's' : ''} — fix before inserting
        </div>
  ...
  // Render preview table — FIXED: proper table inside scroll wrapper
  ...
  document.getElementById('dbPreviewInfo').innerHTML =
    `<strong>${rows.length.toLocaleString()} rows</strong> · ${headers.length} columns · previewing first 5`;
...
async function dbInsert() {
  ...
  try {
    ...
    progressLabel.textContent = `Done! ${inserted.toLocaleString()} rows inserted successfully.`;
